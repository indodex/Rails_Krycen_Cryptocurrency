...
#    ask_price: 'span.ask_section_price'
#    bid_price: 'span.bid_section_price'
#    ask_price: 'span.bid_section_price'
#    bid_price: 'span.ask_section_price'
  ...
  @panelType = ->
    switch @$node.attr('id')
      when 'bid_entry' then 'ask'
      when 'ask_entry' then 'bid'
...
#      jqXHR.abort()
  ...
  @priceHintAlertHide = (event) ->
    @select('positionsLabelSel').fadeOut ->
      $(@).text('')
...
#    price = @select('priceSel').val()
#
#    @select('ask_price').text(parseFloat(((parseFloat(usd_amount) / parseFloat(btc_amount)) * parseFloat(price))).toFixed(2))
