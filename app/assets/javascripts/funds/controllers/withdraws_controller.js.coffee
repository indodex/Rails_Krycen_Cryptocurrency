app.controller 'WithdrawsController', ['$scope', '$stateParams', '$http', '$gon', '$state', 'fundSourceService', 'ngDialog', ($scope, $stateParams, $http, $gon, $state, fundSourceService, ngDialog) ->
...
#      data: {currency: $scope.currency}
    ...
    ctrl.state.transitionTo("manageaddress.currency", {currency: $scope.currency})
    ctrl.selectedCurrency = $scope.currency
    ctrl.currentAction = "manageaddress"
  ...
  @fixingDecimal = ->
    @withdraw.sum = parseFloat(@withdraw.sum.toString().match(/^-?\d+(?:\.\d{0,8})?/)[0])
]
